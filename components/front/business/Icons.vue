<template>
  <div class="flex justify-between items-start">
    <div v-if="hasViewFeatures" class="flex flex-col gap-1 justify-center items-center">
      <icons-outline-eye class="w-6 h-6"/>
      <span class="text-xs">{{ item.views }}</span>
    </div>

    <button v-if="hasScoreFeatures" class="flex flex-col justify-center items-center" @click="voteModal = true">
      <i class="flaticon-029-star text-2xl leading-none"></i>
      <span class="text-xs persian-number">{{ item.score }}</span>
    </button>

    <button v-if="hasChatFeatures" @click="openChat">
      <icons-chat2 class="w-6 h-6" />
    </button>

    <button v-if="hasReportFeatures" @click="reportModal = true">
      <IconsReport class="w-6 h-6"/>
    </button>

    <button v-if="hasShareFeatures" @click="shareModal = true">
      <i class="flaticon-009-share text-2xl leading-none"></i>
    </button>

    <button v-if="hasQrCodeFeatures" @click="qrModal = true">
      <i class="flaticon-086-qr-code text-2xl leading-none"></i>
    </button>

    <button v-if="hasBookmarkFeatures" :class="[item.bookmarked ? 'text-red-550' : '']" @click="$emit('click-bookmark')">
      <i class="flaticon-154-bookmark text-2xl leading-none"></i>
    </button>

    <Modal
      v-if="hasScoreFeatures"
      :show="voteModal"
    ...
    >
      <div class="bg-white sm:shadow-xl p-6 w-full h-screen sm:h-auto relative overflow-y-auto">
        <i
          ...
          @click="voteModal = false"
        ></i>
        <div class="bg-white rounded py-12 px-8 persian-number text-gray-500">
          <div v-for="i in 5" :key="i" class="flex items-center text-sm leading-none mb-3">
            ...
            <div class="flex-grow-1 h-1.5 w-60 bg-gray-200 rounded-full mx-4">
              <div
                ...
                :style="{width: scoreTotal ? item[`score_${6 - i}`] * 100 / scoreTotal : 0 + '%'} "
              ></div>
            </div>
            <span class="w-8">{{
                scoreTotal ? item[`score_${6 - i}`] * 100 / scoreTotal : 0
              }}%
            </span>
          </div>
          <div class="text-gray-300 flex justify-evenly my-4 mx-14" @mouseleave="votePreview = 0">
            <svg
              ...
              @click="$router.push(`/${item.link}/comments?score=${i}`)"
            >
              <path
                v-if="votePreview >= i"
    ...
    >
      <div class="bg-white sm:shadow-xl p-6 w-full h-screen sm:h-auto relative overflow-y-auto">
        <i
          ...
          @click="qrModal = false"
        ></i>
        <div class="bg-white rounded py-12 px-8">
    ...
    >
      <div class="bg-white sm:shadow-xl p-6 w-full h-screen sm:h-auto relative overflow-y-auto">
        ...
        <div class="flex flex-wrap gap-4 justify-center py-8">
          <a :href="`https://telegram.me/share/url?url=${fullPageLink}&text=${item.name}`" target="_blank" class="w-12 h-12 mx-2" @click="shareModal = false">
          ...
          <a :href="`https://wa.me/?text=${fullPageLink}`" target="_blank" class="w-12 h-12 mx-2 rounded-full flex items-center justify-center" style="background-color: #25D366" @click="shareModal = false">
          ...
          <a :href="`https://www.linkedin.com/shareArticle?mini=true&url=${fullPageLink}&title=${item.name}&summary=${item.description}&source=`" target="_blank" class="w-12 h-12 mx-2 rounded-full flex items-center justify-center" style="background-color: #0e76a8" @click="shareModal = false">
            <svg class="w-8 h-8" fill="#fff" viewBox="0 0 40 40">
              <path d="m13.3 31.7h-5v-16.7h5v16.7z m18.4 0h-5v-8.9c0-2.4-0.9-3.5-2.5-3.5-1.3 0-2.1 0.6-2.5 1.9v10.5h-5s0-15 0-16.7h3.9l0.3 3.3h0.1c1-1.6 2.7-2.8 4.9-2.8 1.7 0 3.1 0.5 4.2 1.7 1 1.2 1.6 2.8 1.6 5.1v9.4z m-18.3-20.9c0 1.4-1.1 2.5-2.6 2.5s-2.5-1.1-2.5-2.5 1.1-2.5 2.5-2.5 2.6 1.2 2.6 2.5z"></path>
            </svg>
          </a>

          <a :href="`mailto:?body=${fullPageLink}`" target="_blank" class="w-12 h-12 mx-2 rounded-full flex items-center justify-center" style="background-color: #7d7d7d" @click="shareModal = false">
            <svg class="w-8 h-8" fill="#fff" viewBox="0 0 40 40">
              <path d="m33.4 13.4v-3.4l-13.4 8.4-13.4-8.4v3.4l13.4 8.2z m0-6.8q1.3 0 2.3 1.1t0.9 2.3v20q0 1.3-0.9 2.3t-2.3 1.1h-26.8q-1.3 0-2.3-1.1t-0.9-2.3v-20q0-1.3 0.9-2.3t2.3-1.1h26.8z"></path>
            </svg>
          </a>

          <a :href="`https://twitter.com/intent/tweet?text=${fullPageLink}`" target="_blank" class="w-12 h-12 mx-2 rounded-full flex items-center justify-center" style="background-color: #1DA1F2" @click="shareModal = false">
            <svg class="w-8 h-8" fill="#fff" viewBox="0 0 40 40">
              <path d="m31.5 11.7c1.3-0.8 2.2-2 2.7-3.4-1.4 0.7-2.7 1.2-4 1.4-1.1-1.2-2.6-1.9-4.4-1.9-1.7 0-3.2 0.6-4.4 1.8-1.2 1.2-1.8 2.7-1.8 4.4 0 0.5 0.1 0.9 0.2 1.3-5.1-0.1-9.4-2.3-12.7-6.4-0.6 1-0.9 2.1-0.9 3.1 0 2.2 1 3.9 2.8 5.2-1.1-0.1-2-0.4-2.8-0.8 0 1.5 0.5 2.8 1.4 4 0.9 1.1 2.1 1.8 3.5 2.1-0.5 0.1-1 0.2-1.6 0.2-0.5 0-0.9 0-1.1-0.1 0.4 1.2 1.1 2.3 2.1 3 1.1 0.8 2.3 1.2 3.6 1.3-2.2 1.7-4.7 2.6-7.6 2.6-0.7 0-1.2 0-1.5-0.1 2.8 1.9 6 2.8 9.5 2.8 3.5 0 6.7-0.9 9.4-2.7 2.8-1.8 4.8-4.1 6.1-6.7 1.3-2.6 1.9-5.3 1.9-8.1v-0.8c1.3-0.9 2.3-2 3.1-3.2-1.1 0.5-2.3 0.8-3.5 1z"></path>
            </svg>
          </a>

          <a :href="`https://pinterest.com/pin/create/button/?url=${fullPageLink}&media=${baseUrl+item.avatar}&description=${item.name}`" target="_blank" class="w-12 h-12 mx-2 rounded-full flex items-center justify-center" style="background-color: #E60023" @click="shareModal = false">
            <svg class="w-8 h-8" fill="#fff" viewBox="0 0 40 40">
              <path d="m37.3 20q0 4.7-2.3 8.6t-6.3 6.2-8.6 2.3q-2.4 0-4.8-0.7 1.3-2 1.7-3.6 0.2-0.8 1.2-4.7 0.5 0.8 1.7 1.5t2.5 0.6q2.7 0 4.8-1.5t3.3-4.2 1.2-6.1q0-2.5-1.4-4.7t-3.8-3.7-5.7-1.4q-2.4 0-4.4 0.7t-3.4 1.7-2.5 2.4-1.5 2.9-0.4 3q0 2.4 0.8 4.1t2.7 2.5q0.6 0.3 0.8-0.5 0.1-0.1 0.2-0.6t0.2-0.7q0.1-0.5-0.3-1-1.1-1.3-1.1-3.3 0-3.4 2.3-5.8t6.1-2.5q3.4 0 5.3 1.9t1.9 4.7q0 3.8-1.6 6.5t-3.9 2.6q-1.3 0-2.2-0.9t-0.5-2.4q0.2-0.8 0.6-2.1t0.7-2.3 0.2-1.6q0-1.2-0.6-1.9t-1.7-0.7q-1.4 0-2.3 1.2t-1 3.2q0 1.6 0.6 2.7l-2.2 9.4q-0.4 1.5-0.3 3.9-4.6-2-7.5-6.3t-2.8-9.4q0-4.7 2.3-8.6t6.2-6.2 8.6-2.3 8.6 2.3 6.3 6.2 2.3 8.6z"></path>
            </svg>
          </a>

          <a :href="`https://www.facebook.com/sharer/sharer.php?u=${fullPageLink}`" target="_blank" class="w-12 h-12 mx-2 rounded-full flex items-center justify-center" style="background-color: #4267B2" @click="shareModal = false">
...
export default {
  ...
  computed: {
    hasViewFeatures() {
      return this.item.features?.length && this.item.features.includes('views')
    },
    hasScoreFeatures() {
      return this.item.features?.length && this.item.features.includes('score')
    },
    hasChatFeatures() {
      return this.item.features?.length && this.item.features.includes('chat')
    },
    hasReportFeatures() {
      return this.item.features?.length && this.item.features.includes('report')
    },
    hasShareFeatures() {
      return this.item.features?.length && this.item.features.includes('share')
    },
    hasQrCodeFeatures() {
      return this.item.features?.length && this.item.features.includes('qr_code')
    },
    hasBookmarkFeatures() {
      return this.item.features?.length && this.item.features.includes('bookmark')
    },
    scoreTotal() {
  ...
  methods: {
    ...
    openChat() {
      ...
      this.$router.push(`/panel/chat?business=${this.item.id}`)
    },
    bookmark() {
      if (!this.$store.getters["user/loggedIn"]) {
        alert('برای ذخیره کسب و کار لازم است به حساب کاربری خود وارد شوید.')
